...
openssl x509 -req -days 9999 -CAcreateserial -in $RUCIOHOME/etc/ruciouser.csr -CA /etc/grid-security/certificates/rucio_ca.pem -CAkey /etc/grid-security/certificates/rucio_ca.key.pem -out $RUCIOHOME/etc/usercert.pem
